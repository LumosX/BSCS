...
class Attack:
    ...
    def __str__(self):
        return self.name + ", " + self.type + ", dice: " + self.dice.value + ", damage: " + self.damage.value + " (level, if a spell: " + str(self.spellLevel) + ")"
...
#
    def Attack(self, targetActor, attackName = None):
        ...
        attackString = self.name + " attacks " + targetActor.name + (". " if attack.name == "Default" else " using " + attack.name + ". ")
        ...
        attackString += (" Dealt " + str(damageDealt) + " - " + str(targetActor.ARM) + " = " + str(attackDamage) + " damage, " + 
            targetActor.name + " " + str(targetActor.currentHP) + "/" + str(targetActor.END) + ".")
        return attackString
        
    def __str__(self):
        return (self.name + " (" + str(self.currentHP) + "/" + str(self.END) + "): FPR " + str(self.FPR) + ", AWA " + str(self.AWA) + 
...
def PrintCommands():
    ...
    print("> 'a <X> <Y> [name]' (or 'attack') = makes actor X attack actor Y with attack [name] (default one if none specified). X and Y can be the IDs from the status list, or (partial) names.")
    ...
    print("> 'exec <code>' = executes the code supplied afterwards. Can be used to modify the battlefield directly. Use at your own risk.")
...
## TODO: wrap this in a "with" clause
file = open("combatLog.txt", "w", encoding="utf-8")
